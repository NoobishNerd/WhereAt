<template>
  <div class="row">
    <div class="col-sm-6">
      <div class="form-group">
        <select
          ...
          id="sltTables"
        >
          <option v-for="table in tables" v-bind:key="table.id_table"
            >Mesa {{ table.id_table }} | {{ table.capacity }} pessoas</option
          >
        </select>
      </div>
    </div>
    ...
    <div class="col-sm-6">
      ...
      <button class="mt-4" @click="removeTable" id="removeButtontables">
        Remover Mesa
      </button>
    </div>
  </div>
...
export default {
  ...
  created: async function() {
    this.tables = await restaurantService.getRestaurantTables(
      this.restaurantId
    );
  },
