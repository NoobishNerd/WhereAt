  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-sm-1 mt-3">
        <button @click="addTag()" id="addTagBtn" class="mt-4 px-3 py-2">
          Adicionar
        </button>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-sm-2" v-for="tag in tags" v-bind:key="tag.id_tag + 1000">
        <p
          v-if="tag.tag_main == true"
          class="text-center main"
          style="color: white;"
        >
          {{ tag.tag_name }}
          <span class="ml-2"
            ><button @click="removeTag(tag.id_tag)" id="removeTagBtn">
              X
            </button></span
          >
        </p>
        <p
          style="color:black"
          v-if="tag.tag_main == false"
          class="text-center side"
        >
          {{ tag.tag_name }}
          <span class="ml-2"
            ><button @click="removeTag(tag.id_tag)" id="removeTagBtnWhite">
              X
            </button></span
          >
        </p>
...
export default {
  ...
  methods: {
    async changeMainTag() {
      await restaurantService.updateRestaurantTag(
        this.restaurantId,
        this.newMainTag
      );
      this.tags = await restaurantService.getRestaurantTags(this.restaurantId);
