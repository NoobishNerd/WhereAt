  ...
  <div class="container-fluid">
    <div v-if="this.$store.state.loggedUser.type != 'restaurant'">
        ...
        <div class="row">
          ...
          <div class="col-sm-3 text-right">
            <button
              @click="cancelComment"
              id="cancelBtn"
              class="mt-0 px-1 mr-2 py-1 ml-2"
            >
              Cancelar
            </button>
            <button
      ...
      <div v-else class="row mt-3">
        <h5 class="text-center mt-2 ml-3">
          Precisa de estar autenticado para comentar!
        </h5>
      </div>
    </div>
    ...
    <div class="row" v-for="comment in comments" v-bind:key="comment.id_user">
      ...
      <div class="col-sm-9 pl-0">
        <h6 class="pt-1 mb-1" id="brownBoldText" style="color:#f17526">
          {{ comment.username }}
        </h6>
  ...
  methods: {
    ...
    async addComment() {
      if (this.newRating == "") {
        ...
        swal.fire(
          "Comentário",
          "Contas restaurante não podem comentar!",
          "warning"
        );
      } else {
        ...
        if (
          commentResponse != "Não tem Permissão para Comentar neste Restaurante"
        ) {
          this.comments.push({
