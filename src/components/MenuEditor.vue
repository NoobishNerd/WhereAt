  ...
  <div class="container-fluid">
    <div class="row pt-2">
      ...
      <div class="col-sm-3 text-center">
        <button @click="addItem" class="mt-4 py-3" id="addItemBtn">
          Adicionar
        </button>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-sm-12">
        <h4>Entradas</h4>
        <hr />
        <div v-for="menuItem in menu" v-bind:key="menuItem.id_dish + 0">
          <!-- Eu não queria nada usar esta resolução, mas é a unica forma de usar v-for várias vezes no mesmo array e na mesma página-->
          <h6 style="color:black" v-if="menuItem.category == 'Entradas'">
            {{ menuItem.dish_name }} - {{ menuItem.price }}€
            <button
              @click="removeItem(menuItem.id_dish)"
              id="removeItemBtn"
              class="ml-2"
            >
              X
        ...
        <div v-for="menuItem in menu" v-bind:key="menuItem.id_dish + 1000">
          <h6 style="color:black" v-if="menuItem.category == 'Bebidas'">
            {{ menuItem.dish_name }} - {{ menuItem.price }}€
            <button
              @click="removeItem(menuItem.id_dish)"
              id="removeItemBtn"
        ...
        <div v-for="menuItem in menu" v-bind:key="menuItem.id_dish + 2000">
          <h6 style="color:black" v-if="menuItem.category == 'Pratos de Carne'">
            {{ menuItem.dish_name }} - {{ menuItem.price }}€
            <button
              @click="removeItem(menuItem.id_dish)"
              id="removeItemBtn"
        ...
        <div v-for="menuItem in menu" v-bind:key="menuItem.id_dish + 3000">
          <h6 style="color:black" v-if="menuItem.category == 'Pratos de Peixe'">
            {{ menuItem.dish_name }} - {{ menuItem.price }}€
            <button
              @click="removeItem(menuItem.id_dish)"
              id="removeItemBtn"
        ...
        <div v-for="menuItem in menu" v-bind:key="menuItem.id_dish + 4000">
          <h6
            style="color:black"
            v-if="menuItem.category == 'Pratos Vegetarianos'"
          >
            {{ menuItem.dish_name }} - {{ menuItem.price }}€
            <button
              @click="removeItem(menuItem.id_dish)"
        ...
        <div v-for="menuItem in menu" v-bind:key="menuItem.id_dish + 5000">
          <h6 style="color:black" v-if="menuItem.category == 'Sobremesas'">
            {{ menuItem.dish_name }} - {{ menuItem.price }}€
            <button
              @click="removeItem(menuItem.id_dish)"
              id="removeItemBtn"
  ...
  methods: {
    async addItem() {
      if (this.item == "" || this.itemType == "" || this.price == "") {
        ...
        this.menu = await restaurantService.getRestaurantMenu(
          this.restaurantId
        );
      }
    },
